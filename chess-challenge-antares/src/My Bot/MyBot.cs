...
public class MyBot : IChessBot
{
    ...
    */
    ...
    (ulong ttKey, Move ttMove)[] transpositionTable = new (ulong ttKey, Move ttMove)[ttLength];
    ...
    public int Negamax(Board board, Timer timer, int alpha, int beta, int depth, int ply)
    {
        ...
        var ttEntry = transpositionTable[board.ZobristKey % ttLength];
        ...
        if (qsearch)
        {
            ...
            if (board.IsRepeatedPosition() || board.FiftyMoveCounter >= 100) return 0;
        ...
        for (int i = 0; i < moves.Length; i++)
        {
            Move move = moves[i];
            int score = 0;
            if (ttEntry.ttKey == board.ZobristKey && move == ttEntry.ttMove) score += 500000;
            if (move.IsCapture)
        ...
        for (int currentCount = 0; currentCount < moves.Length; currentCount++)
        {
            ...
            if (currentCount >= 3 && depth >= 3 && scores[currentCount] < 10000 && !inCheck)
            {
        ...
        transpositionTable[board.ZobristKey % ttLength] = (board.ZobristKey, bestMove);
